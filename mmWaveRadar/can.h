...
public:
    ...
    void OpenCANThread(int devType, int canIndex, int baud, int mode);
    void CloseCANThread();

    void ReceiveCANThread();
    void TransmitCANThread(CAN_OBJ psend);
    void parseData(CAN_OBJ data);
public:
...
signals:
    ...
    void send1data(CAN_OBJ data);
    void sendSignal(bool flag, CAN_OBJ data);
    void sendObstacleInfoList(QList<ObstacleInfo> obsList);
};
