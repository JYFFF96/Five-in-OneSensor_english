...
QString radarStatusToString(const RadarStatus& s) {
    ...
    return QString("NVM Read Status: %1, NVM Write Status: %2, Maximum Range: %3 m, Radar ID: %4, Sort Index: %5, Radar Power: %6, Output Type: %7, CAN Baud Rate: %8, RCS Sensitivity: %9, Calibration Status: %10")
        .arg(read)
        .arg(write)
...
QString parseBbstacleData(VCI_CAN_OBJ data)
{
    ...
    text = QString("Target ID: %1, Longitudinal Distance: %2 m, Lateral Distance: %3 m, Longitudinal Velocity: %4 m/s, Lateral Velocity: %5 m/s, Dynamic Property: %6, Type: %7, RCS: %8")
                       .arg(id)
                       .arg(distLong)
                       .arg(distLat)
                       .arg(vrelLong)
